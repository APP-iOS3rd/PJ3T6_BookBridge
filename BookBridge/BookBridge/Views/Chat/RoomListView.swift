    ...
    var body: some View {
        List {
            ForEach(viewModel.searchChatRoomList()) { chatRoom in
                ZStack {
                    NavigationLink {
                        ChatMessageView(chatRoomListId: chatRoom.id, noticeBoardId: chatRoom.noticeBoardId, noticeBoardTitle: chatRoom.noticeBoardTitle, partnerId: chatRoom.partnerId, partnerImageURL: viewModel.getPartnerImageIndex(partnerId: chatRoom.partnerId, noticeBoardId: chatRoom.noticeBoardId), uid: chatRoom.userId)
                    } label: {
                        EmptyView()
                    }
                    .opacity(0.0)
