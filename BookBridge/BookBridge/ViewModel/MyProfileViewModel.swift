...
extension MyProfileViewModel {
    func checkingSameNickname(nickname: String, userSaveImage: (String, UIImage), completion: @escaping((Bool, String)) -> ()) {
        if selectImage != userSaveImage.1 && userNickname != nickname {  //둘다 변경
        ...
        } else if selectImage != userSaveImage.1 {                      //이미지만 변경
            self.saveProfileImage { urlString in
                self.db.collection("User").document(self.userManager.uid).updateData([
                    "profileURL": urlString
                ])
